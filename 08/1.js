...
const calculateAntinodeCoordinates = (coordCombos, grid) => {
    ...
    for (const key of coordCombos.keys()) {
        ...
        for (const coordPair of coords) {
            ...
            const antiNode1 = [coordPair[0][0] + dRow, coordPair[0][1] + dCol];
            const antiNode2 = [coordPair[1][0] - dRow, coordPair[1][1] - dCol];

            if (isInBounds(grid, antiNode1)) { antinodeCoords.add(`${antiNode1[0]},${antiNode1[1]}`); }
            if (isInBounds(grid, antiNode2)) { antinodeCoords.add(`${antiNode2[0]},${antiNode2[1]}`); }
        }
    }
